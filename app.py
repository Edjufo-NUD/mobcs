...
print("KEY PREFIX:", SUPABASE_KEY[:15] if SUPABASE_KEY else None)
    ...
    try:
        ...
        try:
            ...
            if exists:
                ...
                cursor.execute(
                    "UPDATE users SET username = %s, age = %s, gender = %s WHERE id = %s",
                    (username, age, gender, user_id)
                )
            else:
                # Insert new user
                cursor.execute(
                    "INSERT INTO users (id, username, email, age, gender, role, status) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                    (user_id, username, email, age, gender, "user", "Enabled")
                )
            conn.commit()
            cursor.close()
            conn.close()
    ...
    try:
        ...
        return jsonify({
            ...
            'feedback': {
                ...
                'simple_feedback': feedback.get('simple_feedback', []),
                'no_body_detected': feedback.get('no_body_detected', False),
                'no_movement_detected': feedback.get('no_movement_detected', False),
                'video_error': feedback.get('video_error', False)
            }
        })
...
def compare_dance(user_poses, ref_poses):
    ...
    # --- Simple body part feedback (just the worst landmarks) ---
    simple_feedback = []
    for landmark_name, error_score in worst_landmarks[:3]:  # Top 3 worst landmarks
        simple_feedback.append(landmark_name)

    feedback = {
        "worst_landmarks": worst_landmarks[:3],
        "frame_feedback": top_feedback,
        "angle_errors": avg_angle_errors,
        "simple_feedback": simple_feedback  # <-- Simple landmark names list
    }
